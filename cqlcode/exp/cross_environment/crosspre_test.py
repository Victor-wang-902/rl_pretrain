...
def main():
    ...
    settings = [
        'env', '', MUJOCO_4_ENVS[:1], # this contains 4 mujoco envs: ant, halfcheetah, hopper, walker2d
        'dataset', '', MUJOCO_3_DATASETS[:1], # this contains 3 mujoco datasets: medium, medium-expert, medium-replay
        ...
        "policy_with_text", '', [True, False], # whether to use text discription in policy networks

        "offline_data_ratio", '', [0.01], # data ratio for offline learning
        "pretrain_data_ratio", '', [0.01], # data ratio for pretraining

        'qf_hidden_layer', 'ql', [3], # hidden layers for q networks
        "policy_hidden_layer", 'pl', [3], # hidden layers for policy networks

        'n_pretrain_step_per_epoch', 'preUps', [2], # pretrain updates per epoch
        'n_pretrain_epochs', 'preEp', [2], # pretrain epochs
        'n_epochs', '', [1], # offline epochs
        'n_train_step_per_epoch', '', [2],  # offline updates per epoch
        'pretrain_lr', '', [1e-4], # pretrain learning rate for q networks and policy networks (if applicable)
        'finetune_qf_lr', '', [2e-4], # offline learning rate for q networks
        'finetune_policy_lr', '', [3e-4], # offline learning rate for policy networks
        'seed', '', [4069], # random seeds
    ]
