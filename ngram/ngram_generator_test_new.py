...
class RandomTokenGenerator:
    def __init__(
        ...
        if self.mode == 2 or self.mode == 3:
            self.distribution_seeder = default_rng(self.seed)
            self.distribution = self.distribution_seeder.random(self.vocab).reshape(1,-1)
    ...
    def generate_worker(self, worker_id, total_itr, max_length, batch_size, results_queue, event):
        ...
        if self.mode == 0:
            ...
            distribution = softmax(self.distribution).reshape(-1)
            generated = self.sample_seeder[worker_id].multinomial(1, distribution, (batch_size, max_length)).argmax(-1)
        else:
