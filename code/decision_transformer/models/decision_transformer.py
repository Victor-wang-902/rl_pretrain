...
class DecisionTransformerForward(TrajectoryModel):
    ...
    def forward(
        ...
        '''
        #print("x", x.shape)
        action_preds, return_preds = None, None
        if self.return_placeholder:
            state_preds = self.predict_state(x[:, 2])  # predict next action given state
            #print("after", state_preds.shape)
        else:
            state_preds = self.predict_state(x[:, 1])  # predict next action given state
...
class DecisionTransformerForwardDiscrete(TrajectoryModel):  # in this model, only self.transformer is kept similar to language pretraining
    def __init__(
        self,
        args,
        n_state,
        ...
        **kwargs
        ...
        super().__init__(n_state, n_act, max_length=max_length)
        self.return_placeholder = return_placeholder
        self.hidden_size = hidden_size
        # note: the only difference between this GPT2Model and the default Huggingface version
        # is that the positional embeddings are removed (since we'll add those ourselves)
        ...
        # note: we don't predict states or returns for the paper
        if args["share_input_output_proj"]:
            self.predict_state = lambda x: F.linear(x, self.embed_state.weight)  # remove transpose as it is embedding
            self.predict_return = lambda x: F.linear(x, self.embed_return.weight.t())
            self.predict_action = lambda x: F.tanh(
                F.linear(x, self.embed_action.weight)
            )
        else:
            self.predict_state = torch.nn.Linear(hidden_size, self.state_dim)
            self.predict_action = nn.Sequential(
