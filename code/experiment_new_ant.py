...
import random
import sys
from utils import get_normalized_score, get_unnormalized_score
...
def experiment(
    ...
    if variant["calculate_extra"]:
        ...
        if env_name == "hopper":
            ...
            env = gym.make("Ant-v3")
            max_ep_len = 1000
            #env_targets = [2000, 3500, 3800, 4000, 4500]
            #env_targets = [4800, 5000, 5500, 6000]
            #env_targets = [6500, 7000, 8000, 9000, 10000]
            #env_targets = [2000, 3500, 3800, 4000, 4500,4800, 5000, 5500, 6000, 6500, 7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000, 15000, 16000, 17000, 18000, 19000, 20000]
            env_targets = [2000, 5000]
            scale = 1000.0
            final_target = 5000
    ...
    for iter in range(variant["max_iters"]):
        ...
        for target in env_targets:
            logger.log_tabular("current_itr_eval_" + str(target) + "_return_mean", outputs["evaluation/target_" + str(target) + "_return_mean"])
            logger.log_tabular("current_itr_eval_" + str(target) + "_return_std", outputs["evaluation/target_" + str(target) + "_return_std"])
            logger.log_tabular("current_itr_eval_" + str(target) + "_length_mean", outputs["evaluation/target_" + str(target) + "_length_mean"])
            logger.log_tabular("current_itr_eval_" + str(target) + "_length_std", outputs["evaluation/target_" + str(target) + "_length_std"])
    ...
    if variant["target_return_finding"]:
        with open(os.path.join(variant["outdir"], "progress.csv"), "r") as f:
            df = pd.read_csv(f, delimiter="\t", header=0)
            env_targets_ret = dict()
            for target in env_targets:
                env_targets_ret["final_" + str(target) + "_return_mean"] = df["current_itr_eval_" + str(target) + "_return_mean"].iloc[-1]
                env_targets_ret["best_" + str(target) + "_return_mean"] = max(df["current_itr_eval_" + str(target) + "_return_mean"])
                env_targets_ret["last_four_" + str(target) + "_return_mean"] = df["current_itr_eval_" + str(target) + "_return_mean"].iloc[-4:].mean()
                env_targets_ret["convergence_" + str(target) + "_step"] = df["Steps"].iloc[df["current_itr_eval_" + str(target) + "_return_mean"].ge(env_targets_ret["best_" + str(target) + "_return_mean"] - get_unnormalized_score(env_name, 2.0)).idxmax()]
                env_targets_ret["best_" + str(target) + "_step"] = df["Steps"][df["current_itr_eval_" + str(target) + "_return_mean"] == env_targets_ret["best_" + str(target) + "_return_mean"]].iat[0]
        with open(os.path.join(variant["outdir"], "target_finding.json"), "w") as f:
            env_targets_ret = convert_json(env_targets_ret)
            json.dump(env_targets_ret, f, indent=4)
...
def set_dt_args(args_to_parse=None):
    ...
    parser.add_argument("--target_return_finding", action="store_true", default=False)
