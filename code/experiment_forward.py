...
from decision_transformer.models.decision_transformer import DecisionTransformer, DecisionTransformerForward
from decision_transformer.models.mlp_bc import MLPBCModel
from decision_transformer.training.act_trainer_new import ActTrainer
from decision_transformer.training.seq_trainer_new import SequenceTrainer, StateTrainer
...
def experiment(
    ...
    if not variant["discrete_mdp"]:
        ...
        dataset_path = variant["mdp_path"]
        temp = data_path.split("_")
        n_state = int(temp[2][2:])
        n_act = int(temp[3][2:])
    model_type = variant["model_type"]
    ...
    if not variant["discrete_mdp"]:
        ...
        print("=" * 50)
        print(f"Starting new experiment: discrete mdp {dataset_path}")
        print(f"{len(traj_lens)} trajectories, {num_timesteps} timesteps found")
        print("=" * 50)
    ...
    # used to reweight sampling so we sample according to timesteps instead of trajectories
    p_sample = traj_lens[sorted_inds] / sum(traj_lens[sorted_inds])
    if variant["inner_shuffle"]:
        shuffle_random_state = np.random.default_rng(variant["seed"])
    def get_batch(batch_size=256, max_len=K):
        batch_inds = np.random.choice(
            np.arange(num_trajectories),
            size=batch_size,
        ...
        for i in range(batch_size):
            ...
            tlen = s[-1].shape[1]
            #if i == 1:
            #print("before s", s[-1][:,:,0])
            #print("before a", a[-1][:,:,0])
            if variant["inner_shuffle"]:
                shuffle_random_state = np.random.default_rng(i * 5 + variant["seed"])
                shuffle_random_state.shuffle(s[-1], axis=1)
                shuffle_random_state = np.random.default_rng(i * 5 + 1 + variant["seed"])
                shuffle_random_state.shuffle(a[-1], axis=1)
                shuffle_random_state = np.random.default_rng(i * 5 + 2 + variant["seed"])
                shuffle_random_state.shuffle(r[-1], axis=1)
                shuffle_random_state = np.random.default_rng(i * 5 + 3 + variant["seed"])
                shuffle_random_state.shuffle(d[-1], axis=1)
                shuffle_random_state = np.random.default_rng(i * 5 + 4 + variant["seed"])
                shuffle_random_state.shuffle(rtg[-1], axis=1)
    ...
    if model_type == "dt":
        if variant["forward_dynamics"]:
            model = DecisionTransformerForward(
                args=variant,
                state_dim=state_dim,
                act_dim=act_dim,
                max_length=K,
                ...
                resid_pdrop=variant["dropout"],
                attn_pdrop=0.1,
                return_placeholder=variant["return_placeholder"]
            )
        elif variant["discrete_mdp"]:
            model = DecisionTransformerForwardDiscrete(
                args=variant,
                state_dim=state_dim,
                act_dim=act_dim,
                max_length=K,
    ...
    for iter in range(variant["max_iters"]):
        ...
        print("HI2!")
        if not variant["forward_dynamics"]:
...
def set_dt_args(args_to_parse=None):
    ...
    parser.add_argument("--forward_dynamics", action="store_true", default=False)
    parser.add_argument("--return_placeholder", action="store_true", default=False)

    parser.add_argument("--inner_shuffle", action="store_true", default=False)
    parser.add_argument("--discrete_mdp", action="store_true", default=False)
